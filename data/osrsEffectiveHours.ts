...
const INEFFICIENT_COMBAT_EFFECTIVE_HOUR_SKILLS = new Set([
  'attack',
  'strength',
  'defence',
  'hitpoints',
...
function shouldSuppressEffectiveHourSkill(skill: string) {
  return INEFFICIENT_COMBAT_EFFECTIVE_HOUR_SKILLS.has(skill);
}
...
export function calculateOsrsEffectiveHoursFromGains(
  ...
  const bySkill = Object.entries(gainsBySkill)
    .filter(([skill, xp]) => skill !== 'overall' && xp > 0 && !shouldSuppressEffectiveHourSkill(skill))
